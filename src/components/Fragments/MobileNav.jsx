...
const MobileNav = ({ openNavigation, isLoggedIn, toggleNavigation, activeMenu }) => {
  ...
  return (
    <div ref={mobileNavRef} className="absolute flex flex-col items-end w-full pb-1 text-dark-2 bg-white z-50 transition-all duration-300 md:hidden shadow-lg rounded-lg">
      {menuItems.map((item, index) => (
        <RouterLink
          key={index}
          to={item.to}
          onClick={toggleNavigation}
          className={`flex font-lato font-bold text-md cursor-pointer border-t border-bg-border w-full py-4 pl-4 ${activeMenu === item.label ? "text-primary-400" : "text-dark-2 hover:text-primary-400"}`}
        >
          {item.label}
        </RouterLink>
      ))}
      ...
      {isLoggedIn ? (
        <div
          ...
          className="flex flex-row font-lato font-semibold text-tertiary-400 hover:text-tertiary-300 border-y border-bg-border w-full py-4 pl-4 cursor-pointer"
        >
          Keluar
          <svg
