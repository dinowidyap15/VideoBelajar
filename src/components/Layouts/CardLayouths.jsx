    ...
    <div className="overflow-hidden border border-bg-border rounded-xl bg-white flex flex-col items-start relative p-4 gap-4">
      <div className="flex flex-row gap-3 md:flex-col">
        <img
          ...
          className="w-20 h-20 md:w-auto md:h-auto md:rounded-xl object-cover rounded-lg md:object-contain"
        />
        <div className="flex flex-col md:flex-col justify-between">
          ...
          <div className="flex items-center gap-2 mt-auto">
            ...
            <div className="flex flex-col justify-between">
              <h4 className="font-lato font-bold text-dark-1 md:text-md text-sm">
                {card.name}
              </h4>
              <p className="font-lato font-regular text-dark-2 md:text-sm text-xs">
                {card.role} <span className="hidden sm:inline">di</span>{" "}
                <span className="font-lato font-bold text-dark-2 sm:text-sm text-xs hidden sm:inline">
                  {card.company}
                </span>
              </p>
            </div>
      ...
      <div className="flex justify-end w-full">
        <p className="font-poppins font-semibold md:text-xl text-lg text-primary-400">
          Rp{card.price},00
        </p>
      </div>
      {showActions && (
